...
if (isset($_POST['deposit'])) {
    ...
    $stmt = $connection->prepare("INSERT INTO deposit (uuid, user_id, account_type, amount) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("sisd", $uuid, $id, $account_type, $amount);
    $stmt->execute();


    // Redirect to payment_method page
    header("Location: payment_method/?deposit_id=" . $uuid);
    exit;
}
?>
                    ...
                    <div class="col l4 offset-l4 s12 center">
                        ...
                        <div class="card-panel">
                            <form method="POST" autocomplete="off">
                                ...
                                <div class="row">
                                    <div>
                                        <div class="input-field"><span class=" prefix">USD</span><label class="" for="amount">amount</label>

                                            <input inputmode="decimal" type="number" id="amount" min="3" max="" step="any" name="amount" class="" required="" value="">
                                        </div>
                                    </div>
                                    <div class="input-field undefined"><label class="active">to</label>
                                        <select id="" name="account_type" class="browser-default undefined">
                                            <option value="trading_balance" selected>Trading Account</option>
                                            <option value="bitcoin_balance">Bitcoin Account</option>
                                            <option value="ethereum_balance">Ethereum Account</option>
                                            <option value="dogecoin_balance">Dogecoin Account</option>
                                            <option value="binance_coin_balance">Binance Coin Account</option>
                                            <option value="cosmos_atom_balance">Cosmos (ATOM) Account</option>
                                            <option value="stablecoin_balance">Stable Coin Account</option>
                                            <option value="usdt_balance">USDT Account</option>
                                            <option value="solana_balance">Solana Account</option>
                                            <option value="cardano_ada_balance">Cardano (ADA) Account</option>

                                        </select>
                                    </div>
                                </div>
                                <div>
                                    <button type="submit" name="deposit" class=" newbtn ">Proceed</button>
                                    <style>
                                        .newbtn {
                                            background-color: #ff7700;
                                            border-color: #122b40;
