...
function addCalculatorItem(coin) {
    ...
    reloadCalculatorItemAction();
    $.post($('body').attr('hrefapp') + '/app/modules/kr-calculator/src/actions/addCalculatorItem.php', {
        symbol: coin.symbol,
        name: coin.name
    }).done(function(data) {
...
function calculate(val) {
    ...
    $('.kr-calculatorside-lc > section').each(function() {
        if (!$(this).hasClass('kr-calculatorside-lcsc')) {
            ...
            fromsymbol = $(this).attr('symbol');
        }
    });
    symbollist = symbollist.slice(0, -1);

    // Post calculator for get rate (args : from, to, value)
    $.post($('body').attr('hrefapp') + '/app/modules/kr-calculator/src/actions/getRates.php', {
