...
function _setAliveActivity() {
    ...
    clearTimeout(checkActivityUserStatus);
    checkActivityUserStatus = null;
    chatUserStatus = 1;
    checkActivityUserStatus = setTimeout(function() {
        _checkActivity();
    }, 10000);
...
function _syncChatBar() {
    clearTimeout(syncBarTimeout);
    syncBarTimeout = null;

    if ($('.kr-chat-right').length == 0) return false;

    $.get($('body').attr('hrefapp') + '/app/modules/kr-chat/src/actions/syncRightBar.php', {
