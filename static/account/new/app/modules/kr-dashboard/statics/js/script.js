    ...
    $('.kr-top-graphlist-item').each(function() {
        ...
        $(this).find('.kr-top-graphlist-closeb').off('click').click(function(e) {
            ...
            if ($(this).parent().attr('container').length > 0 && $('.kr-dash-pan-cry[container="' + $(this).parent().attr('container') + '"]').length > 0) {
                ...
                $(this).parent().remove();
                $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/deleteTopList.php', {
                    topitem: topItemID
                }).done(function(data) {
...
function addTopListGraph(coin, callback = null) {
    ...
    $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/addTopList.php', {
        symbol: coin.symbol,
...
function initTopListItemTouch() {
    ...
    $('ul.kr-top-graphlist').off('mousedown').on("mousedown", function(e) {
        curDown = true;
        curXPos = e.pageX;
        e.preventDefault();
    });
