...
function updateListCoinGraph(query = null, graph = null, callbackclick = null, reset = true, startat = 0) {
    ...
    clearTimeout(listCoinGraphQuery);
    listCoinGraphQuery = null;
    listCoinGraphQuery = setTimeout(function() {

        // Get list coin
        $.get($('body').attr('hrefapp') + "/app/modules/kr-dashboard/src/actions/getCoinList.php", {
            q: query,
            s: startat
        }).done(function(data) {
...
function showDashboardManager() {
    $('.kr-change-dashboard-selector').show();
}

/**
 * Hide dashboard change configuration
 */
function hideDashboardManager() {
    $('.kr-change-dashboard-selector').hide();
}
...
function changeGraphType(type = "candlestick", container) {
    ...
    $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/changeTypeGraph.php', {
        graph_id: $('#' + container).attr('graph-id'),
        type: type
    }).done(function(data) {
        let jsonRes = jQuery.parseJSON(data);
        if (jsonRes.error == 1) {
...
function editIndicator(container, indicator, index) {
    ...
    // Get indicator form
    $.post($('body').attr('hrefapp') + "/app/modules/kr-dashboard/src/actions/editIndicator.php", {
        graph: container,
        indic: indicator,
        key: index
    }).done(function(data) {
