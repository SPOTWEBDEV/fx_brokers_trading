...
function addChartNotification(chart, symbol, val, currency, market = "CCCAGG") {
    ...
    $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/createNotification.php', {
        symb: symbol,
        currency: currency,
        market: market,
        value: val
...
function createNewNotification(symbol, currency, val = null, market = "CCCAGG") {
    ...
    $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/createAlert.php', {
        symb: symbol,
        currency: currency,
        click: val,
        market: market
...
function initNewNotificationPopupControllers() {
    ...
    $('.kr-list-notification-coin').find('li').find('.lnr-trash').off('click').click(function() {
        ...
        closeAddNotificationPopup();
        $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/deleteNotification.php', {
            notifid: $(this).parent().attr('kr-notification-id')
        }).done(function(data) {
