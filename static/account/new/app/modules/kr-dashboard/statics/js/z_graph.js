...
function changeGraph(dataCoin, currency, container, topitem, replace = false, market = "CCCAGG") {
    ...
    // Save new chart data
    $.post($('body').attr('hrefapp') + "/app/modules/kr-dashboard/src/actions/changeGraph.php", {
        container: container,
        topitem: topitem,
        coinsymbol: dataCoin.symbol,
        currency: currency,
        market: market
    }).done(function(data) {
        // Parse json data
...
function removeGraph(container) {
    ...
    $.post($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/deleteGraph.php', {
        container: container
    }).done(function(data) {
        let response = jQuery.parseJSON(data);
        if (response.error == 1) {
...
function loadChart(symbol, callback, container, currency, market = "CCCAGG") {
    ...
    // Load chart content
    $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChartContent.php', {
        coin: symbol,
        container: container,
        currency: currency,
        market: market
...
function loadChartData(symbol, currency, market, fnc = null, container) {
    // Get chart data
    $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChart.php', {
        coin: symbol,
        currency: currency,
        market: market,
        type: 'load'
...
function syncChart(container) {
    ...
    $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/loadChart.php', {
        coin: symbol,
        type: 'update',
        market: $('#' + container).attr('market'),
        currency: $('#' + container).attr('currency')
    }).done(function(data) {
