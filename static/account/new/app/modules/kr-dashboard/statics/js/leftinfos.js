...
function loadLeftInfosIsInitied() {
    return loadInfosInitied;
}
...
function updateLeftInfosOrderBook(market = "binance", symbol = "ETH", currency = "BTC") {
    ...
    if ($('.kr-wtchl').hasClass('kr-wtchl-lowd') || $('.kr-infoscurrencylf-orderbook-coin').attr('kr-ob-force') == "true") {
        ...
        $.get($('body').attr('hrefapp') + '/app/modules/kr-dashboard/src/actions/getOrderBook.php', {
            market: market,
            symbol: symbol,
            currency: currency
        }).done(function(data) {
...
function loadOrderGraphList(symbol, currency, market = null) {
    ...
    $.get($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/getOrderList.php', {
        symbol: symbol,
        currency: currency,
        market: market
    }).done(function(data) {
...
function _startBinanceSocketOrderBook(symbol, currency, depth = "") {
    socketBinanceBookOrder = new WebSocket('wss://stream.binance.us:9443/ws/' + symbol.toLowerCase() + '' + currency.toLowerCase() + '@depth' + depth);
