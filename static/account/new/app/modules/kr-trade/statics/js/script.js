...
function _cancelOrder(orderid) {
    ...
    $.post($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/cancelTrade.php', {
        orderid: orderid
    }).done(function(data) {
...
function toggleOrderbook() {
    ...
    if ($('.kr-orderbookside').hasClass('kr-orderbookside-show')) {
        ...
        clearTimeout(timeOutOrderBook);
        timeOutOrderBook = null;
        closeOrderInfos();
    }
    checkGraphResize();
}
...
function initWidthdrawMethod(type) {
    ...
    $.get($('body').attr('hrefapp') + '/app/modules/kr-trade/views/initWidthdraw.php', {
        type: type
    }).done(function(data) {
