$(document).ready(function() {
    ...
    $('.kr-wallet-balance-show-list').off('click').click(function() {
        if ($(this).hasClass('kr-wallet-balance-show-list-native')) {
            ...
            changeView('trade', 'balances', {
                exchange: $(this).attr('kr-balance-exchange')
            });
        }
    ...
    $('.kr-wallet-top-change').find('[kr-wallet-exch-name]').off('click').click(function() {
        $('.kr-wallet-top > section').css('display', 'none');
        $.post($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/changeMainThirdparty.php', {
            exchange: $(this).attr('kr-wallet-exch-name')
        }).done(function(data) {
...
function _askWidthdraw(symbol = null) {
    _closeCreditForm();
    $.get($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/askWidthdraw.php', {
        symbol: symbol
    }).done(function(data) {
...
function _initCreditPopup() {
    ...
    $('[kr-balance-credit]').off('click').click(function() {
        ...
        if (type == "practice") {
            $.post($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/depositBalance.php', {
                bid: $(this).attr('kr-balance-idc')
            }).done(function(data) {
                let jsonRes = jQuery.parseJSON(data);
                if (jsonRes.error == 1) {
                    showAlert('Oops', jsonRes.msg, 'error');
    ...
    $('[kr-charges-payment]').off('click').click(function() {
        ...
        if (ptype == "creditcard") {
            _loadCreditForm('depositCreditCard', {
            ...
            $.post($('body').attr('hrefapp') + '/app/modules/kr-payment/src/actions/deposit/processOther.php', {
                type: ptype,
                amount: paymentAmount,
                currency: $('.kr-balance-credit-drel-cont').attr('kr-bssymbol')
            }).done(function(data) {
                let jsonRes = jQuery.parseJSON(data);
                if (jsonRes.error == 1) {
                    showAlert('Oops', jsonRes.msg, 'error');
...
function _changeWalletBalance(bid) {
    $.post($('body').attr('hrefapp') + '/app/modules/kr-trade/src/actions/changeBalance.php', {
        bid: bid
    }).done(function(data) {
        let jsonRes = jQuery.parseJSON(data);
        if (jsonRes.error == 1) {
...
function _checkPaymentStatus(type, time, openbox = null) {
    ...
    if (openbox && !openbox.closed) {
        ...
        $.get($('body').attr('hrefapp') + '/app/modules/kr-payment/src/actions/deposit/checkPaymentStatus.php', {
            type: type,
            time: time
        }).done(function(data) {
...
function _checkCoinGatePayment(t) {
    clearTimeout(checkCoinGateTimeout);
    checkCoinGateTimeout = null;
    $.get($('body').attr('hrefapp') + '/app/modules/kr-payment/src/actions/deposit/checkCoingate.php', {
        t: t
    }).done(function(data) {
